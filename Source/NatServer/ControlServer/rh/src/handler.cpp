...
int Client_Handler::process ()
{
    ...
    if(!ACE_OS::memcmp(&m_buf, "KONY", 4))
    {
		if(mKO_ProControl.Process((const char *)&m_buf, bytes_read)) {
			if (this->peer().send(SendData.c_str(), SendData.length()) == -1) {
				return -1;
			}
		}
	}
